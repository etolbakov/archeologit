...
At the moment only 4 commands are available:
 - `b <step>` (a.k.a. `back <step>`) : move the git pointer to the previous `<step>` `sha`; the default `<step>` is 1;
 - `n <step>` (a.k.a. `next <step>`) : move the git pointer to the next `<step>` `sha`; the default `<step>` is 1;
...
Supported commands are case-insensitive: `b`/`B`, `n`/`N`, `q`/`Q`, `h`/`H` work interchangeably.
